...
def get_id(y, x, width):
    return (y-1)*width+x

def floyd(height, width, observation_list):
    mat = np.zeros((height,width))
    for y1 in range(height):
        for x1 in range(width):
            for y2 in range(height):
                for x2 in range(width):
                    id1 = get_id(y1, x1, width)
                    id2 = get_id(y2, x2, width)
                    if (id1 != id2):
    ...
    for y in range(height):
        for x in range(width):
            id1 = get_id(y, x, width)
            if (y == 0) : id2 = id1 + (height - 1) * width
            else : id2 = id1 - width
            mat[id1][id2] = 1
            if (x == 0) : id2 = id1 + width - 1
            else : id2 = id1 - 1
            mat[id1][id2] = 1
            if (y == height - 1) : id2 = id1 - (height - 1) * width
            else : id2 = id1 + width
            mat[id1][id2] = 1
            if (x == width - 1) : id2 = id1 - width + 1
            else : id2 = id1 + 1
            mat[id1][id2] = 1
    for i, (snake_y, snake_x) in observation_list[2]:
        id1 = get_id(snake_y,snake_x,width)
        for y in range(height):
            for x in range(width):
                id2 = get_id(y, x ,width)
                mat[id1][id2] = math.inf
                mat[id2][id1] = math.inf
    for i, (snake_y, snake_x) in observation_list[3]:
        id1 = get_id(snake_y,snake_x,width)
        for y in range(height):
            for x in range(width):
                id2 = get_id(y, x ,width)
...
def F_calc_greedy_hacker(observation_list):
    beans_position = observation_list[1]
    your_dist, your_bean = get_min_bean_distance(observation_list[3][0][0], observation_list[3][0][1], beans_position)
    my_dist, my_bean =  get_min_bean_distance(observation_list[2][0][0],observation_list[2][0][1],beans_position)
...
def my_controller(observation_list, action_space_list, is_act_continuous=False):
    ...
    for i in snakes[1]:
        state[i[0], i[1]] = 3
    actions = greedy_snake_zhy(state, beans, snakes, width, height, [mysnake])
    player = []
    each = [0] * 4
    each[actions[0]] = 1
    player.append(each)
